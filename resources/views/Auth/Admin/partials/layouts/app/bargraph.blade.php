  ...
  <div class="flex justify-between border-gray-200 dark:border-gray-700">
    <div class="flex items-center">
      ...
      <div>
        <h5 class="leading-none text-2xl font-bold text-gray-900 dark:text-white pb-1">Quiz Activity</h5>
        <p class="text-sm font-normal text-gray-500 dark:text-gray-400">Unique users taking quizzes (daily)</p>
      </div>
    </div>
    <div>
  ...
  <div id="column-chart"></div>
    <div class="grid grid-cols-1 items-center border-gray-200 border-t dark:border-gray-700 justify-between">
      <div class="flex justify-between items-center pt-5">
        ...
        <span class="uppercase text-sm font-semibold inline-flex items-center rounded-lg text-blue-600 px-3 py-2">Last 7 days</span>
      </div>
    </div>
<script>
const options = {
  ...
  series: [
    {
      name: "Users",
      color: "#1A56DB",
      data: (@json($quizDailyLabels ?? []).map((x, i) => ({ x, y: (@json($quizDailyCounts ?? [])[i] ?? 0) })))
    },
  ],
  chart: {
    type: "bar",
