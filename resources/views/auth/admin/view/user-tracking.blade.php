...
<main class="p-4 md:ml-64 min-h-screen pt-5 pb-12 bg-gray-100 dark:bg-gray-900">
    <!-- Header -->
        ...
        <div class="flex items-center gap-2 w-full md:w-auto">
            <form action="{{ route('users.tracking') }}" method="GET" class="flex flex-col md:flex-row gap-2 md:gap-3 w-full">
                <div class="relative w-full md:w-80">
                    ...
                    />
                    <button type="submit" class="absolute top-0 right-0 p-2.5 text-sm font-medium h-full text-white bg-primary-600 rounded-r-lg border border-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                ...
                <div class="relative w-full md:w-56">
                    ...
                    <select
                        id="sort"
                        name="sort"
                        class="block w-full p-2.5 text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400">
                        <option value="recent" @selected(request('sort', 'recent') === 'recent')>Updated (Newest)</option>
                        <option value="name_asc" @selected(request('sort') === 'name_asc')>Name A → Z</option>
                        <option value="name_desc" @selected(request('sort') === 'name_desc')>Name Z → A</option>
                        <option value="journals_desc" @selected(request('sort') === 'journals_desc')>Most Journals</option>
                    </select>
                </div>
            </form>
        </div>
